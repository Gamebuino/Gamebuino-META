...
uint8_t loading_time_max = 40;  // Time in frames you have to hold HOME and MENU
...
void MetaMode::update() {
	...
	if (gb.buttons.repeat(Button::home, 0) && gb.buttons.repeat(Button::menu, 0) && !active &&
	    !anim_running && !anim_unhandled_running) {
		loading_timer++;
...
void MetaMode::drawLoadingLines(uint8_t percentage) {
	static uint8_t color_offset = 0;  // Makes the color shifting happen
	color_offset = (color_offset + 2) % NUMBER_OF_COLORS_LINE_PATTERN;  // '+1' moves too slow, so we use '+2'
